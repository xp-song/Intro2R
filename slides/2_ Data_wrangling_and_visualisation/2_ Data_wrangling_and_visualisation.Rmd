...
__[It's your turn!](#exercise)__

__[Further applications](#resources)__
...
__Use `summary()` or `str()` to examine the data summary/structure, or `head()` to see the first few rows__
...
.left-column[ 
### <span style="color:grey">Datasets</span>
### <span style="color:grey">Join</span>
### Save
]
.right-column[
__Let's save this summary table `ctry_data` on our computer!__
```{r eval = FALSE}
...
- Use `ggplot` to plot the columns `country`, `respop10k` and `label`
- We can assign the plot to a new variable `ctryplot`
...
#<continued>
 theme(legend.position = "none", 
  ...
  annotate("label", x = 7, y = ctry_data$respop10k[ctry_data$iso3=="USA"], 
           label = "USA",size=3, color="black") +
  annotate("label", x = 7, y = mean(ctry_data$respop10k), 
           label = "Overall mean", size=3, color="black")
```
...
```
--
- Finally, let's print the figure!
```{r eval = FALSE}
print(ctryplot)
```
...
- Let's make a scatter plot with columns `respop10k` (x) against the innovation `index` (y)
...
ggplot(ctry_data, aes(x = respop10k, 
  ...
  geom_text_repel(size = 3.5) + #plot text (country label), from library(ggrepel)
...
```{r fig.height = 5, fig.width = 8, echo = FALSE, dev='svg'}
ggplot(ctry_data, aes(x = respop10k, 
                     y = index, 
                     color = label, 
                     label = country)) + 
...
- Switzerland was the most innovative country in 2018

- There were more respondents in Singapore as a proportion of the population
...
__We can test the correlation between `respop10k` and the innovation `index`__
...
- Strong correlation
- Remove objects from R using rm()
...
rm(ctry_data, ctry_n, ctryplot, innov, multi, pops, cols)
```
...
__Load dataset `diamonds` from the package `ggplot2`__
...
__Summarise columns in dataset__
...
__Code the following figure__

```{r echo = FALSE, dev='svg', message=FALSE, fig.height=5}
#jpeg(filename = "../../output/diamond_price.jpeg", width = 1800, height = 1500, res = 300)
diamonds %>%
  ggplot(aes(x = price)) + 
  geom_histogram()
#dev.off()
...
```{r eval = FALSE, echo=FALSE, dev='svg', fig.height=5}
#a bit laggy. Load image instead
#jpeg(filename = "../../output/diamond_scatter.jpeg", width = 1800, height = 1500, res = 300)
diamonds %>%
  ggplot(aes(x = price, 
             y = carat,
             color = cut)) + 
...
```{r eval = TRUE, echo = FALSE, out.height=500, fig.align='center'}
knitr::include_graphics("images/diamond_scatter.jpeg")
```
...
--
```{r eval = FALSE}
  geom_point(alpha = 0.5) + #alpha adjusts transparency
```
--
```{r eval = FALSE}
  #customisation
  scale_color_brewer(type = "seq") #sequential color scale
```
...
__[Resources and further applications](#resources)__
