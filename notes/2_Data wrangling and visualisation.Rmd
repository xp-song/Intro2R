...
Here, we are interested in the column `` `In which country do you currently reside?` ``. We want a summarised table with the count of respondents per country.

- Abbreviate the colname to `country`
- `count()` the number of of observations per `country`
- `filter()` resulting table to exclude (`!`) those who selected "Other" or "I do not wish to disclose my location".
  - Exclude factors in `country` (rows) that match the names `%in%` the specified vector
- `mutate()` table to add a new column named `iso3` containing the country code
  - Factor names in `country` are mapped to a three-letter code using the function in `library(countrycode)`
...
Next, we use the package [highcharter](http://jkunst.com/highcharter) to plot an interactive map.
```{r fig.height=6}
highchart() %>%
